...
class AccountAuth {
  ...
  async auth(credentials) {
    try {
      ...
      if (!token)
        throw new Error("[Account Authorization] Cannot get XSRF TOKEN!");

      /**
       * Sending login form
       */
      let { retry } = await this.launcher.http.sendPost(
      ...
      if (retry) return await this.auth(credentials);

      let { response } = await this.launcher.http.sendGet(
       ...
       */
      const authData = await this.exchangeCode(code);

      if (!authData)
        throw new Error(
          "[Account Authorization] Cannot exchange code and receive authData!"
        );
  ...
  async register(options) {
    try {
      ...
      if (generalExceptionError.length) {
        generalExceptionError = generalExceptionError
          .eq(0)
          .text()
          .trim();
        throw new Error(
          `Error while registration. Message: ${generalExceptionError}`
        );
      }

      if (fieldValidationErrorElements.length) {
        throw new Error(
          `Error while registration. Field: ${fieldValidationErrorElements
            .eq(0)
            .attr("for")} Message: ${fieldValidationErrorElements.eq(0).text()}`
        );
      }
      ...
      if (!exchangeCode)
        throw new Error("[Account Authorization] Cannot get exchange code!");
  ...
  async submitTwoFactorCode(token, twoFactorFormElement, twoFactorCode) {
    const twoFactorForm = {
      ...
      alternateMfaMethods: twoFactorFormElement
        .find('input[name="alternateMfaMethods"]')
        .val(),
      displayName: twoFactorFormElement
        .find('input[name="epic_username"]')
        .val(),
    ...
    if (!twoFactorCode) {
      twoFactorForm.twoFactorCode = await new Promise(resolve => {
        Prompt.question(
          `Enter two factor code (${twoFactorForm.mfaMethod}): `,
          resolve
        );
      });
    } else {
      switch (typeof twoFactorCode) {
        ...
        default:
          throw new Error(
            "`twoFactorCode` parameter must be `string`, `number` or `function`."
          );
      }
    }
    ...
    const { data } = await this.launcher.http.sendPost(
      ...
      false,
      {
        "X-XSRF-TOKEN": this.launcher.http.jar
          .getCookies(`${ENDPOINT.LOGIN_FRONTEND}/login/doLauncherLogin`)
    ...
    const fieldValidationErrorElement = $(
      'label[for="twoFactorCode"].fieldValidationError'
    );
  ...
  async getXSRF(location) {
    ...
    return this.launcher.http.jar
      .getCookies(`https://www.epicgames.com/id/api/csrf`)
      .find(cookie => cookie.key == "XSRF-TOKEN").value;
  }
  ...
  async getExchangeCode(url) {
    const {
      response: { body }
    } = await this.launcher.http.sendGet(url, "launcher", {}, false);
    ...
    try {
      ...
      if (data) {
        ...
        if (this.launcher.communicator) {
          ...
          this.launcher.debug.print(
            "Communicator: Reconnected with new account's access token."
          );
        }
